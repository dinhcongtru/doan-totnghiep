  ...
  <div class="coverSibar" :class="coverSibar">
    ...
    <span class="clickOutside" @click="ToggleDropdown()">
      <span class="input-light">
        <input :readonly="searchable" type="text" autocomplete="off" ref="input" :disabled="disabled" :class="styleInput"
          :placeholder="placeholder" :value="currentValue" :tabindex="tabInput" />
      </span>
      <i class="fa-solid fa-angle-left"></i>
      <div class="option-dropdown" v-if="isOpenDropdown">
        <ul>
          <li v-for="(item, index) in itemData" :key="index" :class="[item == currentValue && active]"
            @click="onDropdownValue(item)">{{ item }}
          </li>
        </ul>
      </div>
    </span>
